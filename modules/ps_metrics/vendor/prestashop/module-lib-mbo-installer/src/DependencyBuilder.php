...
class DependencyBuilder
{
    ...
    public function areDependenciesMet()
    {
        ...
        foreach ($dependencies as $dependencyName => $dependency) {
            if (!\array_key_exists('installed', $dependency) || !\array_key_exists('enabled', $dependency) || \false === $dependency['installed'] || \false === $dependency['enabled']) {
                return \false;
            }
        }
        return \true;
    ...
    protected function handleMboInstallation()
    {
        ...
        $data = [Installer::MODULE_NAME => ['status' => \true]];
        try {
            ...
            $data[Installer::MODULE_NAME] = ['status' => \false, 'msg' => $e->getMessage()];
        }
    ...
    {
        return ['module_display_name' => (string) $this->module->displayName, 'module_name' => (string) $this->module->name, 'module_version' => (string) $this->module->version, 'ps_version' => (string) _PS_VERSION_, 'php_version' => (string) \PHP_VERSION, 'locale' => $this->getLocale(), 'dependencies' => $this->getDependencies(\true)];
    }
    /**
     * @return string
     */
    ...
    {
        $context = \ps_metrics_module_v4_1_2\ContextCore::getContext();
        if ($context !== null && $context->employee !== null) {
            $locale = \ps_metrics_module_v4_1_2\DbCore::getInstance()->getValue(\sprintf('SELECT `locale` FROM `%slang` WHERE `id_lang`=%s', _DB_PREFIX_, pSQL((string) $context->employee->id_lang)));
        }
        if (empty($locale)) {
            return 'en-GB';
        }
    ...
    protected function buildRoutesForModule($moduleName)
    {
        ...
        foreach (['install', 'enable', 'upgrade'] as $action) {
            $route = $this->router->generate('admin_module_manage_action', ['action' => $action, 'module_name' => $moduleName]);
            if (\is_string($route)) {
                $urls[$action] = $route;
            }
        }
    ...
    {
        global $kernel;
        if (!$kernel instanceof \ps_metrics_module_v4_1_2\AppKernel) {
    ...
    protected function addMboInDependencies($addRoutes = \false)
    {
        ...
        $mboStatus = (new Presenter())->present();
        $specification = ['current_version' => (string) $mboStatus['version'], 'installed' => (bool) $mboStatus['isInstalled'], 'enabled' => (bool) $mboStatus['isEnabled'], 'name' => Installer::MODULE_NAME];
        if (!$addRoutes) {
            return $specification;
        }
        return \array_merge($specification, $this->buildRoutesForModule(Installer::MODULE_NAME));
    }
    /**
     * @return bool
     */
    ...
    private function getDependencies($addRoutes = \false)
    {
        ...
        if (empty($dependenciesContent['dependencies'])) {
            ...
            return [Installer::MODULE_NAME => $mboDependency];
        }
        if ($this->isMboNeeded() && !isset($dependenciesContent['dependencies'][Installer::MODULE_NAME])) {
            $dependenciesContent['dependencies'][] = ['name' => Installer::MODULE_NAME];
        }
        $dependencies = [];
        foreach ($dependenciesContent['dependencies'] as $dependency) {
            ...
            $dependencyData = \ps_metrics_module_v4_1_2\DbCore::getInstance()->getRow(\sprintf('SELECT `id_module`, `active`, `version` FROM `%smodule` WHERE `name` = "%s"', _DB_PREFIX_, pSQL((string) $dependency['name'])));
            if ($dependencyData && \is_array($dependencyData) && !empty($dependencyData['id_module'])) {
                ...
                $isModuleActiveForAllShops = (bool) \ps_metrics_module_v4_1_2\DbCore::getInstance()->getValue(\sprintf('SELECT id_module
                            FROM `%smodule_shop`
                            WHERE id_module=%d AND id_shop IN (%s)
                            GROUP BY id_module
                            HAVING COUNT(*)=%d', _DB_PREFIX_, (int) $dependencyData['id_module'], \implode(', ', \array_map('intval', \Shop::getContextListShopID())), (int) \count(\Shop::getContextListShopID())));
                $dependencyData['active'] = $isModuleActiveForAllShops;
            }
            if ($addRoutes) {
                $dependencies[$dependency['name']] = \array_merge($dependency, $this->buildRoutesForModule($dependency['name']));
            } else {
            ...
            $dependencies[$dependency['name']] = \array_merge($dependencies[$dependency['name']], ['installed' => \true, 'enabled' => isset($dependencyData['active']) && (bool) $dependencyData['active'], 'current_version' => isset($dependencyData['version']) ? $dependencyData['version'] : null]);
        }
        return $dependencies;
    }
    /**
    ...
    private function getDependenciesSpecification()
    {
        ...
        if ($fileContent = \file_get_contents($dependencyFile)) {
            $dependenciesContent = \json_decode($fileContent, \true);
        }
        if (!isset($dependenciesContent) || !\is_array($dependenciesContent) || !\array_key_exists('dependencies', $dependenciesContent) || \json_last_error() != \JSON_ERROR_NONE) {
